...
{
    public GameObject raycastNotification;
    public float raycastDelay;
    public float interactDistance;
    public LayerMask interactMask;

    private RaycastHit groundRaycastHit;
    public Vector3 currentGroundRaycastNormal;
    ...
    private void GroundedCheck()
    {
        ...
        if (Grounded && crouchToggled && _speed > 3f)
        {
            if (Physics.Raycast(transform.position, -Vector3.up, out groundRaycastHit, Mathf.Infinity, GroundLayers))
                currentGroundRaycastNormal = groundRaycastHit.normal;
            else
                currentGroundRaycastNormal = Vector3.zero;

            slopeAngle = Vector3.Angle(transform.forward, currentGroundRaycastNormal);
    ...
    void SetRaycastNotifier(string text)
    {
        raycastNotification.transform.GetChild(0).GetChild(0).GetComponent<Text>().text = text;
        StopCoroutine("ShowRaycast");
        StartCoroutine("ShowRaycast");
    }

    IEnumerator ShowRaycast()
    {
        raycastNotification.SetActive(true);

        yield return new WaitForSecondsRealtime(raycastDelay);

        raycastNotification.SetActive(false);
    }
    ...
    private void Move()
    {
        ...
        if (_input.move != Vector2.zero) //TODO Add deadzone for horizontal and forward movements so the player can strafe in a line easier
        {
        ...
        if (_hasAnimator)
        {
            _animator.SetFloat(_animIDSpeedX, _animationBlend * _input.move.x);
            _animator.SetFloat(_animIDSpeedY, _animationBlend * _input.move.y);
        }
    }
    ...
    private void OnDrawGizmos()
    {
        if (!_mainCamera)
            return;
        Transform targetCamera = _mainCamera.transform;
        Gizmos.color = Color.green;
        Gizmos.DrawRay(CinemachineCameraTarget.transform.position, targetCamera.forward * interactDistance);
    }
}

public enum PlayerState { Playing, Paused, InConversation, Other }